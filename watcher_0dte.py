...
def watch_discord_screen(last_processed_text):
    """Captures the screen and scans for fresh Diamond signals."""
    with mss.MSS() as sct:
        monitor = {"top": 120, "left": 50, "width": 1200, "height": 750}
        img = np.array(sct.grab(monitor))
        gray = cv2.cvtColor(img, cv2.COLOR_BGRA2GRAY)

        # Convert text to uppercase immediately for consistent matching
        text = pytesseract.image_to_string(gray).upper()

        # 1. Anti-Spam: Don't re-process identical screen text
        ...
        if "DIAMOND" in text and "ROUGH" not in text:
            ...
            # 3. Freshness Verification: Parse timestamp and timezone
            # Matches formats like: AT 2026-09-04 15:30:00 ET or AT 2026-09-03 07:40:00 PDT
            time_match = re.search(
                r'AT\s+(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})\s*(ET|EDT|EST|PDT|PST)?',
                text
            )
            if time_match:
                raw_dt_str = time_match.group(1)
                tz_label = time_match.group(2) or "ET"

                try:
                    parsed_dt = datetime.strptime(raw_dt_str, "%Y-%m-%d %H:%M:%S")

                    # Normalize Pacific Time to Eastern (+3 hours)
                    if "PDT" in tz_label or "PST" in tz_label:
                        parsed_dt += timedelta(hours=3)

                    # Compare against system clock (Eastern Time)
                    now = datetime.now()
                    age_seconds = (now - parsed_dt).total_seconds()

                    # Drop stale signals or out-of-sync future timestamps
                    if age_seconds > MAX_SIGNAL_AGE_SECONDS:
                        print(f"⏳ [STALE SIGNAL IGNORED]: Signal is {age_seconds:.0f}s old.")
                        return None, None, text
                    if age_seconds < -15:
                        print(f"⚠️ [CLOCK SKEW]: Signal is timestamped in the future ({age_seconds:.0f}s). Skipping.")
                        return None, None, text

                except Exception as parse_err:
                    print(f"⚠️ Timestamp parse error: {parse_err}. Skipping to avoid stale execution.")
                    return None, None, text
            else:
                # If no timestamp can be read from the alert, reject it
                return None, None, text

            # 4. Check if signal is for an allowed ticker
            for ticker in ALLOWED_TICKERS:
                if f" {ticker} " in text or f"{ticker} AT" in text:
                    # 5. Directional translation: Long -> CALL, Short -> PUT
                    if "LONG" in text:
...
def get_target_option_contract(ticker, opt_type):
    """
    Scans Webull's 0DTE chain for ticker/opt_type to locate a contract
    with an Ask price within TARGET_PREMIUM_MIN and TARGET_PREMIUM_MAX.
    """
    # -------------------------------------------------------------------------
    # TODO: Connect Webull Market Data option chain query here.
    # Expected return dictionary structure:
    # return {
    #     "strike_price": "550.0",
    #     "option_expire_date": "2026-09-08",
    #     "ask_price": "0.95"
    # }
    # -------------------------------------------------------------------------
    print(f"⚠️ [MARKET DATA MISSING]: Need 0DTE chain lookup for {ticker} {opt_type} (~$1.00 premium).")
    return None
...
def run_0dte_watcher(trade_client, active_account_id):
    """Watches Discord for visual signals and actively manages open positions."""
    state = {}
    last_processed_text = ""
    ...
    while True:
        try:
            # 1. Scan for fresh Buy signals
            ticker_signal, opt_type_signal, current_text = watch_discord_screen(last_processed_text)

            if ticker_signal:
                print(f"\n💎 [FRESH DIAMOND SIGNAL]: {ticker_signal} {opt_type_signal}")

                # Log signal telemetry to Supabase
                ...
                # Store screen text state to prevent repeated buys on same post
            ...
            for pos in positions:
                ...
                # Initialize state tracking for new fills
                if sym not in state:
                    state[sym] = {
                        "initial_qty": qty,
                        "runner_active": False,
                ...
                if trade["pending_sell"]:
                    ...
                    if minutes_pending >= timeout_limit:
                        ...
                        success, cid, limit_set = execute_sell(
                            trade_client, active_account_id, sym, qty, "0DTE Step-Down", pos.get("legs", []), force_price=new_limit
                        )
                        if success:
                ...
                # --- RULE 3: RUNNER MANAGEMENT (AFTER 1 SOLD) ---
                    ...
                    if pnl_pct >= 50.0:
                        ...
                        sell_reason = "Runner Reached Breakeven (0.0%). Securing Initial Gain."
                        sell_qty = qty
                ...
                # --- RULE 4: SINGLE CONTRACT MANAGEMENT ---
                    ...
                    if pnl_pct >= 50.0:
                        ...
                        sell_reason = "Single Contract Fell Back to +30% Floor. Securing Profit."
                        sell_qty = 1
